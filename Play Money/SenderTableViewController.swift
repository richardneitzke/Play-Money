...
class SenderTableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.item == 0 {
            ...
            if delegate.sender.0 == -1 && delegate.sender.1 == "Bank" {
        ...
        //Sets Checkmark behind Cell when it's the currently selected sender in the delegate, removes it when not
        if delegate.sender.0 == indexPath.item-1 && delegate.sender.1 == players.players[indexPath.item-1].name {
            cell!.accessoryType = UITableViewCellAccessoryType.Checkmark
        }
        
        //Disables the Cell if it's Player is already the Receiver
    ...
    //Sets the delegates sender to selected player and pops view
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if indexPath.item == 0 {delegate.sender = (-1, "Bank") } else { delegate.sender = (indexPath.item-1, players.players[indexPath.item-1].name) }
